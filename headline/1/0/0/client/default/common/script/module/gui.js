	...
	$self.gui = new function() //TODO : make user alert on errors
	{
		...
		this.period = function(period) //Swap the display period option
		{
			...
			return $system.network.send($self.info.root + 'server/php/run.php', {task : 'gui.period'}, {id : __selected, mode : period}, $self.gui.span);
		}
		...
		this.view = function(id, address, line) //Show the entry's source page
		{
			...
			return $system.network.send($self.info.root + 'server/php/run.php', {task : 'gui.view'}, {id : id, mode : 1});
		}
	}
