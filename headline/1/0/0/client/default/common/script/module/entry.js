	...
	$self.entry = new function()
	{
		...
		this.category = function(id, category) //Set a category for an entry
		{
			...
			return $system.network.send($self.info.root + 'server/php/run.php', {task : 'entry.category'}, {id : id, category : category});
		}
		...
		this.mark = function(id, node) //Mark a topic
		{
			...
			return $system.network.send($self.info.root + 'server/php/run.php', {task : 'entry.mark'}, {id : id, mode : _mark[id]});
		}
		...
		this.show = function(id) //Show a specific entry by ID
		{
			...
			return $system.network.send($self.info.root + 'server/php/run.php', {task : 'entry.show', id : id}, null, $system.app.method(display, [id]));
		}
	}
