	...
	$self.feed = new function()
	{
		...
		this.get = function(xml, callback) //Get list of feeds
		{
			...
			return $system.network.send($self.info.root + 'server/php/front.php', {task : 'feed.get'}, null, $system.app.method(list, [callback])); //Load the list from the server
		}
		...
		this.sample = function(callback) //Insert sample feeds into user's database
		{
			...
			return $system.network.send($self.info.root + 'server/php/front.php', {task : 'feed.sample'}, {language : $global.user.language}, $system.app.method(list, [callback]));
		}
	}
