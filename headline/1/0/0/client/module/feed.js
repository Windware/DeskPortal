	...
	$self.feed = new function()
	{
		...
		this.add = function(input) //Add a new feed
		{
			...
			$system.network.send($self.info.root + 'server/php/front.php', {task : 'feed.add'}, {address : address}, list);
			return false; //Invalidate the form
		}

		this.get = function(xml) //Get list of feeds - TODO : Make it asynchronous with a callback function
		{
			var log = $system.log.init(_class + '.get');

			if(typeof xml != 'object') //If none passed
			{
				var request = $system.network.send($self.info.root + 'server/php/front.php', {task : 'feed.get'}, null, null);
				if(!request.valid()) return false;

				xml = request.xml; //Use the returned list
			}

			if(typeof xml != 'object') return log.dev($global.log.error, 'dev/object', 'dev/object/solution');
			...
			for(var i = 0; i < feeds.length; i++)
			{
				...
				loaded[id] = {period : $system.dom.attribute(feeds[i], 'period'), year : time[0], month : time[1], week : Math.floor((time[2] - 1) / 7) + 1, day : time[2], description : info, address : $system.dom.attribute(feeds[i], 'address'), newest : newest, since : __feed[id] && __feed[id].since};
				...
				$system.image.set(source, $self.info.devroot + 'graphic/site.png');
		...
		this.remove = function(id) //Remove a feed
		{
			...
			$system.network.send($self.info.root + 'server/php/front.php', {task : 'feed.remove'}, {id : id}, list);
			return false; //Stop the form from submitting
		}

		this.sample = function() //Insert sample feeds into user's database
		{
			var log = $system.log.init(_class + '.sample');

			var language = $system.language.strings($id, 'manual.xml');
			if(!confirm(language['3/confirm'])) return false;
			...
			$system.conf.swap($id, 'conf'); //Swap to configuration panel once to have the config elements created first
			$system.conf.swap($id, 'manual');

			return $system.network.send($self.info.root + 'server/php/front.php', {task : 'feed.sample'}, {language : $global.user.language}, list);
		}
	}
