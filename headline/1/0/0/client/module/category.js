	...
	$self.category = new function()
	{
		...
		this.get = function(indexed, list) //Get the current category listing
		{
			...
			{
				var request = $system.network.send($self.info.root + 'server/php/front.php', {task : 'category.get'}, null, null);
		...
		this.remove = function() //Removes the chosen category
		{
			...
			return $system.network.send($self.info.root + 'server/php/front.php', {task : 'category.remove'}, {category : category}, removal);
		}
		...
		this.set = function(form) //Sets a category
		{
			...
			$system.network.send($self.info.root + 'server/php/front.php', {task : 'category.set'}, {id : form.category.value, name : name}, update);
