...
{
	protected static $_namespace = ['dc' => 'http://purl.org/dc/elements/1.1/']; #XML namespace
	...
	public static function update($feed = null) #Update the feed caches (Supports RSS 0.9/1.0/2.0 and ATOM)
	{
		...
		foreach($query->all() as $row) #Construct the request entities
		{
			...
			$header = $updated ? ['If-Modified-Since' => gmdate('r', $updated)] : [];
			$request[] = ['address' => $row['address'], 'header' => $header];
		...
		$query = ['updated' => $database->prepare("UPDATE {$database->prefix}feed SET updated = :updated WHERE id = :id")];
		...
		foreach($results as $content)
		{
			...
			$query['updated']->run([':id' => $index, ':updated' => $system->date_datetime(time() + mt_rand(0, 600))]);
			...
			$query['set']->run([':id' => $index, ':site' => self::_flatten($link), ':description' => self::_flatten($site->title), ':icon' => $icon]);
			...
			foreach($entries as $headline) #For all of the headline entries in the feed retrieved
			{
				...
				$query['entry']->run([':feed' => $index, ':link' => self::_flatten($page)]); #NOTE : Only finding out duplicates via link to work universally on all type of feeds
				...
				$query['insert']->run([':feed' => $index, ':subject' => self::_flatten($headline->title), ':link' => self::_flatten($page), ':date' => $published, ':description' => self::_flatten($headline->{$summary})]);
			}
		}
