...
class Headline_1_0_0_Feed
{
	...
	public function add() //Add the feed to be retrieved periodically on the server side
	{
		...
		$query->run([':address' => $this->address]);
	...
	public function entry($limiter = '', $values = []) //List headline entries in the feed
	{
		$system = $this->_system;
		$log = $system->log(__METHOD__);
		...
		$parameters = [':id' => $this->id]; //Set the query parameter
		...
		return $query->success ? $query->all() : [];
	}
	...
	public static function get(System_1_0_0_User $user = null) //List the subscribed feeds of an user
	{
		...
		$query->run([':user' => $user->id]); //Get the list of subscribed feeds
		...
		$grep = $parameter = $period = []; //Query param
	...
	public function remove() //Delete the feed and its associated entries
	{
		...
		$query->run([':id' => $this->id]); //Delete the feed
	...
	public static function subscribe($id, System_1_0_0_User $user = null) //Subscribe a user to a feed
	{
		...
		$query->run([':user' => $user->id, ':feed' => $id]);
		...
		$query->run([':feed' => $id, ':user' => $user->id]);
	...
	public static function unsubscribe($id, System_1_0_0_User $user = null) //Unsubscribe a user from a feed
	{
		...
		$query = $database->prepare("DELETE FROM {$database->prefix}feed WHERE id = :id");
		$query->run([':id' => $id]); //Remove the feed to avoid fetching the content remotely anymore

		if(!$query->success) $database->rollback();

		$query = $database->prepare("DELETE FROM {$database->prefix}entry WHERE feed = :id");
		$query->run([':id' => $id]); //Remove entries when no user is subscribed
	...
	{
		return $this->id ? Headline_1_0_0::update($this->address) : false;
	} //Update the feed content

	public function xml($limiter = '', $values = []) //Create XML list of feed entries
	{
		$system = $this->_system;
		$log = $system->log(__METHOD__);
