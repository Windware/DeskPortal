...
class Headline_1_0_0_Entry
{
	...
	protected static function _date(&$system, $year, $month, $day = 0) #Create a date string with padding
	{
		...
		if(!$day) return implode('-', [$year, $month]); #For month specifying, return without the day

		if($day < 10) $day = '0' . ((int) $day); #Pad day
		return implode('-', [$year, $month, $day]); #Add the day and return
	}
	...
	public static function get($param, System_1_0_0_User $user = null) #List of a feed's entries
	{
		...
		$base = [':feed' => $id]; #Query parameters
		$values = []; #Array of values to merge when executing queries
		...
		$query = []; #List of database queries
		...
		{
			$query['user']->run(array_merge([':user' => $user->id, ':entry' => $row['id']], $values)); #Get the entry info
			...
			if($param['unread'] == 1 && !count($values))
			{
				if($pref['seen'] == 1) continue;
			...
			$parts = []; #XML attributes
			$query['detail']->run([':id' => $row['id']]); #Get the entry's detail
			...
			$list[] = ['attributes' => $parts, 'data' => preg_replace('/<.+?>/', ' ', $item['description'])];
		}

		return ['amount' => floor(($amount - 1) / self::$_limit) + 1, 'entry' => $list];
	}
	...
	public static function set($feed, $id, $column, $value, System_1_0_0_User $user = null) #Update an entry's preference
	{
		...
		if(!$feed) #For updating the entries table
		{
			...
			$query->run([':user' => $user->id, ':entry' => $id]);
			...
			if($column == 'category') #For this version, only allow setting a single category by removing the older category set
			{
				...
				if($value) #If setting, insert new row
				{
					...
					$query->run([':user' => $user->id, ':entry' => $id, ':category' => $value]);
			...
			else
			{
				if($exist) #If the entry exists, only update the parameter
				{
					...
					$query->run([':user' => $user->id, ':entry' => $id, ":$column" => $value]);
				}
		...
		else #For updating the feeds table
		{
			...
			$query->run([":$column" => $value, ':user' => $user->id, ':feed' => $id]);
		}
	...
	public static function show($id, System_1_0_0_User $user = null) #Get the entry information on the ID
	{
		...
		$query->run([':id' => $id]); #Get the entry information
		...
		$query->run([':user' => $user->id, ':feed' => $entry['feed']]); #Get the feed's period mode
		...
		$info = ['id' => $feed['feed'], 'period' => (int) $feed['period'], 'year' => $date[0], 'month' => $date[1]];
		$value = [':feed' => $feed['feed']]; #Query parameters
