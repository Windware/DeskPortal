$self.convert = new function()
{
	...
	this.currency = function(value, mode, target)
	{
		...
		var address = 'https://www.google.com/finance/converter?a=%%&from=%%&to=%%'
	...
	this.language = function(value, mode, target)
	{
		...
		var address = 'https://translate.google.com/translate_t?hl=%%#%%|%%|%%'
	...
	this.length = function(value, mode, target)
	{
		...
		{
			case 'm' :
				return _round(base(value))

			case 'mm' :
				return _round(base(value / 1000))

			case 'cm' :
				return _round(base(value / 100))

			case 'km' :
				return _round(base(value * 1000))

			case 'ly' :
				return _round(base(value * 9460730472580800))

			case 'ft' :
				return _round(base(value * 0.3048))

			case 'in' :
				return _round(base(value * 0.0254))
	...
	this.mass = function(value, mode, target)
	{
		...
		var base = function(unit) //Base is 'gram'
		{
			switch(target)
			{
				...
				case 'lb' :
					return unit / 0.002204623

				case 'oz' :
					return unit / 0.0352739619
		...
		switch(mode)
		{
			...
			case 'lb' :
				return _round(base(value * 0.002204623))

			case 'oz' :
				return _round(base(value * 0.0352739619))

			case 't' :
				return _round(base(value * 1000000))
	...
	this.speed = function(value, mode, target)
	{
		...
		switch(mode)
		{
			...
			case 'mph' :
				return _round(base(value * 1.609344))

			case 'Ma' :
				return _round(base(value * 1225))

			case 'kt' :
				return _round(base(value * 1.85200))
	...
	this.temperature = function(value, mode, target)
	{
		...
		switch(mode)
		{
			...
			case 'F' :
				return _round(base((value - 32) * 5 / 9))
		...
		switch(mode)
		{
			...
			case 'month':
				switch(target)
				{
					...
					default :
						return _round(base(value * 30))
				}
				break

			case 'week' :
				return _round(base(value * 7))

			case 'day' :
				return _round(base(value))

			case 'hour' :
				return _round(base(value / 24))

			case 'minute' :
				return _round(base(value / 24 / 60))

			case 'second' :
				return _round(base(value / 24 / 3600))
