	...
	$self.gui = new function() //NOTE - Using node's ID to identify the select box, since IE6 can't define 'name' on a created select box
	{
		...
		this.process = function(index) //Execute the operation (By using individual feature script)
		{
			...
			try { var result = $self[feature][method](form.input.value, mode.value, target); } //Execute the specific feature method

			catch(error) { return log.dev($global.log.error, 'dev/execute', 'dev/execute/solution', [feature, method, error]); }
		...
		this.set = function(index, callback) //Initialize a new bar
		{
			...
			return $system.network.send($self.info.root + 'server/php/run.php', {task : 'gui.set', index : index}, null, $system.app.method(set, [node, callback])); //Get the choices
		}
