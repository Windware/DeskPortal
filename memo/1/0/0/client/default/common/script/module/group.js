$self.group = new function()
{
	...
	this.remove = function() //Removes the chosen group
	{
		...
		return $system.network.send($self.info.root + 'server/php/run.php', {task: 'group.remove'}, {group: group}, removal)
	}
	...
	this.set = function(form) //Sets a group
	{
		...
		$system.network.send($self.info.root + 'server/php/run.php', {task: 'conf.set'}, {
			id  : form.group.value,
			name: name,
		}, update)
	...
	this.update = function(xml) //Update the group listing in various locations
	{
		...
		for(var index in __opened)
		{
			...
			for(var j = 0; j < groups.length; j++) //Set the group list
			{
				...
				form.innerHTML += $system.text.format($self.info.template['choice'], {
					id   : groups[j].id,
					name : groups[j].name,
					check: checked,
				})
