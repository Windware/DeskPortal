	...
	$self.item = new function()
	{
		...
		this.add = function(form) //Adds a new memo
		{
			...
			//Register a new memo
			$system.network.send($self.info.root + 'server/php/run.php', {task : 'item.set'}, {name : title, groups : groups}, load);
		...
		this.save = function(id) //Save the memo content
		{
			...
			return $system.network.send($self.info.root + 'server/php/run.php', {task : 'item.save'}, {id : id, content : content}, confirmation);
		}
		...
		this.show = function(id, group) //Load a memo
		{
			...
			return $system.network.send($self.info.root + 'server/php/run.php', {task : 'item.show', id : id}, null, $system.app.method(load, [id, group, edit]));
		}
	}
