$self.item = new function()
{
	...
	this.add = function(form) //Adds a new memo
	{
		...
		var load = function(request)
		{
			...
			{
				case '2' :
					$system.gui.alert($id, 'user/edit/duplicate', 'user/edit/duplicate/message')
					break //Log about duplicate name
				...
				default :
					$system.gui.alert($id, 'user/edit/error', 'user/edit/error/message')
					break //On error
			}
		...
		//Register a new memo
		$system.network.send($self.info.root + 'server/php/run.php', {task: 'item.set'}, {
			name  : title,
			groups: groups,
		}, load)
	...
	this.get = function(list) //Get the list of current memos
	{
		...
		{
			var request = $system.network.send($self.info.root + 'server/php/run.php', {task: 'item.get'}, null, null)
	...
	this.remove = function(id) //Remove a memo
	{
		...
		//Remove the entry
		return $system.network.send($self.info.root + 'server/php/run.php', {task: 'item.remove'}, {id: id}, load)
	}
	...
	this.save = function(id) //Save the memo content
	{
		...
		return $system.network.send($self.info.root + 'server/php/run.php', {task: 'item.save'}, {
			id     : id,
			content: content,
		}, confirmation)
	}
	...
	this.set = function(id, form) //Submit the edited memo information to the server
	{
		...
		$system.window.fade($id + '_edit_' + id, true, null, true) //Let go of the edit window
		return $system.network.send($self.info.root + 'server/php/run.php', {task: 'item.set'}, param, load)
	}
	...
	this.show = function(id, group) //Load a memo
	{
		...
		return $system.network.send($self.info.root + 'server/php/run.php', {
			task: 'item.show',
			id  : id,
		}, null, $system.app.method(load, [id, group, edit]))
	}
}
