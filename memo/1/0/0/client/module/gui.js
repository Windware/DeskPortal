	...
	$self.gui = new function()
	{
		...
		this.fill = function() //Set the groups in the new memo window
		{
			...
			for(var i = 0; i < groups.length; i++) //Set the group list
				node.innerHTML += $system.text.format($self.info.template.choice, {group : groups[i].id, name : $system.text.escape(groups[i].name), checked : ''});
		}
		...
		this.info = function(id) //Load the info pane
		{
			...
			for(var i = 0; i < groups.length; i++)
			{
				...
				list += $system.text.format($self.info.template.pick, {group : groups[i].id, name : $system.text.escape(groups[i].name), info : id, check : checked});
			}
