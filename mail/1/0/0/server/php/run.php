...
switch($_GET['task'])
{
	case 'account.get' : #Get list of accounts
		...
		print $system->xml_dump($data !== false, 'account', $data, ['user', 'supported']);
	break;
	...
	case 'conf.special' : #Set special folders
		...
		print $system->xml_dump($result !== false && $data !== false, 'account', $data, ['user']);
	break;
	...
	case 'gui.show' : #Mark a mail as read
		$result = Mail_1_0_0_Item::flag([$_POST['id']], 'Seen', true);
		print $system->xml_dump($result);
	break;
	...
	case 'item.get' : #Get list of mails stored in the database
		...
		$xml = $system->xml_node('page', ['total' => $data['page']]);
