...
{ #TODO : Include PEAR packages as local files to avoid code change and to minimize external dependencies - Auth_SASL, Net_POP3, Net_SMTP, Mail, Mail_mime
	...
	protected static $_flag = ['deleted' => 'Deleted', 'seen' => 'Seen', 'marked' => 'Flagged', 'replied' => 'Answered', 'draft' => 'Draft']; #Table column names and corresponding IMAP flags
	...
	protected static $_preview = [500, 5];

	protected static $_storage = ['drafts' => 'Drafts', 'sent' => 'Sent']; #Generic name for draft and sent special folders

	protected static $_type = ['text', 'multipart', 'message', 'application', 'audio', 'image', 'video', 'other']; #Mime categories (Order is important)

	private static $_connection = []; #POP3 connection cache for manual connection instead of using 'imap_' functions (For UIDL supports)
	...
	protected static function _dig($parts, $position = '') #Dig through the multi part message
	{
		...
		foreach($parts as $index => $section) #Pick the position in the structure
		{
			...
			$data = self::$_structure[] = ['name' => $name, 'position' => $position . ($index + 1), 'size' => $section->bytes, 'structure' => $section, 'type' => $mime];
		}
	}
	...
	public static function attachment($id, $mode = 'attachment', System_1_0_0_User $user = null) #Get and send the attachment back
	{
		...
		if(!$query->run([':id' => $attachment['folder'], ':user' => $user->id])) return false;
		...
		list($link, $sequence) = self::find([$attachment['mail']], $user, $base);
	...
	public static function find($list, System_1_0_0_User $user = null, $base = null) #Find sequence number on the mail server for mails by ID
	{
		...
		if(!count($collection = $query->all())) return [null, null]; #When no matching mail is found
		...
		if(!$query->run([':id' => $folder, ':user' => $user->id])) return false;

		$link = Mail_1_0_0_Account::connect($query->column(), $folder, $user); #Connect to the server
		if(!$link) return false;

		$numbers = []; #Returning sequence numbers
		...
		else #When UID/UIDL is not available
		{
			...
			$compare = []; #List of mail signatures on the mail server
	...
	public static function flag($list, $flag, $mode, System_1_0_0_User $user = null) #Flag mails
	{
		...
		if($flag != 'Deleted') #Nothing to flag for 'Deleted' flag to the database
		{
			...
			{
				$target = [];
				$param = [':mode' => $mode ? 1 : 0, ':user' => $user->id];
		...
		if(!$query->run([':id' => $list[0], ':user' => $user->id])) return false; #Pick the account type the mail belongs to
	...
	public static function folder($id, System_1_0_0_User $user = null) #Get folder and account the mail belongs to (TODO - Allow specifying the sort order)
	{
		...
		if(!$query->run([':user' => $user->id, ':folder' => $relation['folder']])) return false; #Get entire list of ID to find on which page the mail belongs to
	...
	public static function get($folder, $page, $order = 'sent', $reverse = true, $marked = false, $unread = false, $search = '', $amount = null, System_1_0_0_User $user = null) #Get list of mails
	{
		...
		$param = [':user' => $user->id, ':folder' => $folder];
		...
		if(is_string($search) && strlen($search))
		{
			foreach(preg_split('/(　|\s)/', $search) as $index => $term) #NOTE - Also splitting 'Japanese full width space' in UTF8
			{
				...
				foreach(['from', 'to', 'cc', 'bcc'] as $target) $limiter .= " OR _$target.name LIKE :s{$index}id $database->escape OR _$target.address LIKE :s{$index}id $database->escape";
			...
			if($limiter) foreach(['from', 'to', 'cc', 'bcc'] as $target) if($order != $target) $foreign .= " LEFT JOIN {$database->prefix}$target as _$target ON id = _$target.mail";
		}
		...
		if(!$query['account']->run([':id' => $folder, ':user' => $user->id])) return false;
		...
		if(count($target))
		{
			...
			foreach(['from', 'to', 'cc', 'bcc'] as $section)
			{
				...
				foreach($query['address']->all() as $row) $mail[$row['mail']][$section][] = ['name' => $row['name'], 'address' => $row['address']];
			}
			...
			foreach($query['attachment']->all() as $row) $mail[$row['mail']]['attachment'][] = ['id' => $row['id'], 'cid' => $row['cid'], 'name' => $row['name'], 'size' => $row['size'], 'type' => $row['type']];
		}
		...
		foreach($all as $row)
		{
			foreach(['from', 'to', 'cc', 'bcc', 'attachment'] as $section) #Construct the mail address and attachment lists
		...
		return ['list' => $list, 'page' => floor($total / ($amount + 1)) + 1];
	}
	...
	public static function image($id, $cid) #Get the embedded image data
	{
		...
		if(!$query->run([':id' => $id, ':cid' => $cid, ':user' => $user->id])) return false;
	...
	public static function move($list, $folder, $copy = false, System_1_0_0_User $user = null) #Move or copy mails to another folder
	{
		...
		if($type == 'imap') #For IMAP, move the mails on the server - TODO : Need different code for cross account move. It now breaks when source account and target account differs.
		{
			...
			if(!$query->run([':id' => $list[0], ':user' => $user->id])) return false; #Get folder name from the first mail item (If another mail belongs to another folder, self::find will fail)
		...
		else #For POP3, update the local database only
		{
			if(!$copy) #On move
			{
				...
				{
					$target = [];
					$param = [':folder' => $folder, ':user' => $user->id];
	...
	public static function send($account, $subject, $body, $to, $cc = null, $bcc = null, $source = null, $attachment = [], $draft = false, $resume = null, System_1_0_0_User $user = null) #Send a mail (Or only save as a draft)
	{
		$system = new System_1_0_0(__FILE__);
		$log = $system->log(__METHOD__);
		...
		if(!$query->run([':id' => $account, ':user' => $user->id])) return 1;

		$row = $query->row();
		$encoded = $list = $toward = []; #List of encoded addresses, list of addresses and raw mail addresses to send to
		...
		$conf = $system->app_conf();
		$mailer = str_replace(['%APP%', '%VERSION%'], [$system->self['name'], str_replace('_', '.', $system->self['version'])], $conf['mailer']);

		$time = time();

		$header = [ #Headers to go inside the mail header section
		...
		$mail = ['body' => $mime->get(['head_charset' => 'utf-8', 'text_charset' => 'utf-8']), 'header' => $mime->headers($header)];
		...
		if(!is_string($name) || !strlen($name)) #If not found
		{
			...
			if(!$query->run([':folder' => $folder, ':id' => $account, ':user' => $user->id])) return 1;
		}
		...
		if(!$draft) #When sending
		{
			...
			$connection = [ #SMTP connection parameters - NOTE : TLS is automatically initiated if the mail server supports it
		...
		if($system->is_digit($resume)) Mail_1_0_0_Item::remove([$resume], false, $user); #If a draft is being saved from a suspended draft, delete the old one. Not stopping if error occurs here.
			...
			try
			{
				...
				foreach(['from', 'to', 'cc', 'bcc'] as $section) #Insert all the addresses in the mail
				{
					foreach($list[$section] as $address => $name)
					{
						...
						if(!$query->run([':mail' => $id, ':name' => $name, ':address' => $address])) throw new Exception();
					}
				}
		...
		if($draft) #If saving a draft, send back the ID of the mail to client to know which older draft to delete on next save
		{
			...
			if(!$query->run([':user' => $user->id, ':folder' => $folder, ':draft' => 1])) return 1; #Get list of mails to find the draft
	...
	public static function show($id, System_1_0_0_User $user = null) #Get the body of a message
	{
		...
		if(!$query->run([':id' => $id, ':user' => $user->id])) return false; #Get the display preference

		$row = $query->row();

		list($preference, $folder, $recent) = [$row['preference'], $row['id'], $row['recent']];
		if(!in_array($preference, ['plain', 'html'])) $preference = 'plain'; #Fallback to plain text

		$query = $database->prepare("SELECT seen, plain, html, subject, encode FROM {$database->prefix}mail WHERE id = :id AND user = :user");
		if(!$query->run([':id' => $id, ':user' => $user->id])) return false; #Get the mail body
		...
		if(!$row['seen']) #Mark as seen if not read (Not minding errors) (NOTE : Mail server flag is updated by another process)
		{
			...
			$query->run([':recent' => $recent - 1, ':id' => $folder, ':user' => $user->id]); #Update the amount of unread mails

			$query = $database->prepare("SELECT recent FROM {$database->prefix}folder WHERE id = :id AND user = :user");
			$query->run([':id' => $folder, ':user' => $user->id]); #Update the amount of unread mails
		}
		...
		else #Send out the plain text version : NOTE - Not making cache files since a long expire header will be sent and cache hit ratio will not be high enough to inflate user database
		{
			...
			$container = str_replace(['%top%', '%id%'], [$system->app_conf('system', 'static', 'root'), $system->self['id']], $system->file_read("{$system->self['root']}/resource/mail.html"));
			$body = str_replace(['%subject%', '%body%'], [htmlspecialchars($row['subject']), str_replace("\n", "<br />\n", htmlspecialchars($row['plain']))], $container); #Dump the content in a container
	...
	public static function special($list, $type, $account = null, System_1_0_0_User $user = null) #Move mails to special folders
	{
		...
		if(!is_array($list) || !in_array($type, ['drafts', 'sent', 'trash']) || $account && !$system->is_digit($account)) return $log->param();
		if(!count($list)) return true;
		...
		if(!$system->is_digit($account))
		{
			...
			if(!$query->run([':id' => $list[0], ':user' => $user->id])) return false; #Target the account of the mail if not specified
		...
		if(!$query->run([':id' => $account, ':user' => $user->id])) return false; #Pick the special folder on the specified account
	...
	public static function text($id, System_1_0_0_User $user = null) #Get the plain text version of a mail
	{
		...
		return $query->run([':id' => $id, ':user' => $user->id]) ? $query->column() : false; #Get the mail body
	}
	...
	public static function update($folder, System_1_0_0_User $user = null) #Store any new messages to the local database
	{
		...
		$query = ['insert' => $database->prepare("INSERT INTO {$database->prefix}mail (user, folder, uid, header, signature, subject, mid, sent, draft, seen, preview, plain, html, encode) VALUES (:user, :folder, :uid, :header, :signature, :subject, :mid, :sent, :draft, :seen, :preview, :plain, :html, :encode)")];

		foreach(['from', 'to', 'cc', 'bcc'] as $section) #Query to insert mail addresses
		...
		$current = []; #List of currently existing UID or signatures
		...
		if($link['type'] == 'imap') #For IMAP
		{
			...
			if(!$query['exist']->run([':user' => $user->id, ':folder' => $folder])) return false; #Get list of existing mails' identifiers
		...
		elseif($link['type'] == 'pop3') #NOTE : Get the UID with an alternative method, since 'imap_search' cannot get UID on POP3
		{
			...
			if($link['info']['supported'])
			{
				$unique = []; #Store the unique ID for each message
				$conf = $system->app_conf();
			...
			if(!$query['exist']->run([':user' => $user->id, ':account' => $account])) return false; #Get list of mail identifiers

			foreach($query['exist']->all() as $row) $current[$row[$part]] = true;
		}

		$exist = []; #List of header md5 sum of mails existing in the mail box
		...
		for($sequence = 1; $sequence <= $content->Nmsgs; $sequence++) #For all of the messages found in the mail box
		{
			...
			$attributes = [':user' => $user->id, ':folder' => $folder, ':uid' => isset($unique[$sequence]) ? $unique[$sequence] : null]; #Mail information parameters
			...
			$addresses = []; #List of 'from', 'to', 'cc' and 'bcc' addresses
			...
			foreach($header as $key => $value) #Get mail information
			{
				...
				if(!is_array($value))
				{
					...
					{
						case 'date' :
							$attributes[$store] = $system->date_datetime(strtotime($attributes[$store]));
						break; #Format the time

						case 'subject' :
							if(!$system->is_text($attributes[$store])) $attributes[$store] = '';
						break; #Avoid null values

						case 'message_id' :
							$attributes[$store] = preg_replace('/^<(.+)>$/', '\1', $attributes[$store]);
						break; #Strip the brackets

						case 'draft' :
							$attributes[$store] = $value == 'X' ? 1 : 0;
						break; #Draft entries

						case 'unseen' :
							$attributes[$store] = $link['type'] == 'pop3' || $value == 'U' ? 0 : 1;
						break; #Unread entries (Make it unread for all mails under POP3)
					}
				}
				else #Array parameters are list of addresses
				{
					...
					foreach($value as $index => $inner) #Store the mail addresses
					{
						...
						$addresses[$key][] = [':name' => $parts['personal'] ? $parts['personal'] : null, ':address' => $check[] = $full];
					}
				}
			}
				...
				if($current[$attributes[':signature']]) #Check mail existence by header signature
				{
					if($link['type'] == 'imap')
					{
						foreach(self::$_flag as $column => $mark) if($query[$column]) #Flag the states
							$query[$column]->run([":$column" => $attributes[':' . strtolower($mark)] ? 0 : 1, ':user' => $user->id, ':folder' => $folder, ':signature' => $attributes[':signature']]);
					}
			...
			$attachment = $body = $data = []; #Body data
			...
			foreach(['plain', 'html'] as $type) #NOTE : Need to check 'plain' first to get 'preview'
			{
				...
				{
					case 3 :
						$body[$type] = base64_decode($body[$type]);
					break;

					case 4 :
						$body[$type] = quoted_printable_decode($body[$type]);
					break;
				}
				...
				if($encoding = mb_detect_encoding($body[$type])) #Detect manually first to avoid mails with false or no encoding specified
				{
					...
					else #Keep the encoding as is for HTML version
					{
						...
						{
							case 'ASCII' :
								$attributes[':encode'] = 'us-ascii';
							break;

							case 'JIS' :
								$attributes[':encode'] = 'iso-2022-jp';
							break;

							case 'SJIS' :
							case 'SJIS-win' :
								$attributes[':encode'] = 'shift_jis';
							break;

							case 'EUC-JP' :
							case 'eucJP-win' :
								$attributes[':encode'] = 'euc-jp';
							break;

							default :
								$attributes[':encode'] = strtolower($encoding);
							break;
						}
					}
				}
				else #If it cannot detect, use the encoding specified in the mail if any
				{
					foreach($data[$type]->parameters as $param) #Check for character set
					{
						...
						if($type == 'plain')
						{
							if($encoding != 'us-ascii' && $encoding != 'utf-8') $body[$type] = mb_convert_encoding($body[$type], 'utf-8', $encoding);
						} #Convert to UTF8 if it isn't
				...
				elseif(!preg_match('/\S/', $attributes[':preview'])) #If no plain text is available
				{
					if($attributes[':encode']) #If encoding could be detected for HTML version (Already attempted to detect manually above)
					{
						...
						foreach([':plain', ':preview'] as $section)
						{
			...
			if($link['type'] == 'pop3' && !$query['loaded']->run([':user' => $user->id, ':account' => $account, ":$part" => $attributes[":$part"]]))
				return $database->rollback() && false; #For POP3, remember the downloaded mail

			foreach(['from', 'to', 'cc', 'bcc'] as $section) #Insert the addresses
			{
			...
			foreach($attachment as $file) $query['attachment']->run([':mail' => $id, ':cid' => preg_replace('/^<(.+)>$/', '\1', $file['structure']->id), ':name' => $file['name'], ':position' => $file['position'], ':size' => $file['size'], ':type' => $file['type']]);
		...
		if($link['type'] == 'imap') #On IMAP, do a batch update of mail flags and deletion
		{
			if($link['info']['supported']) #If UID is available
			{
				...
				if(!$query['flag']->run([':user' => $user->id, ':folder' => $folder])) return false; #Select all messages in the folder
				...
				foreach($query['flag']->all() as $row)
				{
					foreach(self::$_flag as $column => $mark) #Prepare the difference for update
					{
						if($row[$column])
						{
							if(!$state[$column][$row['uid']]) $set[0][$column][] = $row['id'];
						} #For removing flag
				...
				foreach(self::$_flag as $column => $mark)
				{
					...
					for($i = 0; $i <= 1; $i++) #For mails both flags set and unset
					{
						...
						{
							$target = [];
							$param = [":$column" => $i, ':user' => $user->id, ':folder' => $folder];
			...
			elseif(!count($exist)) #If no mail is on the server for this folder
			{
				...
				if(!$query->run([':user' => $user->id, ':folder' => $folder])) return false; #Pick all mails in the folder
		...
		{ #FIXME - Sometimes the wrong records get deleted, making mails to be downloaded again
			...
			if(!is_array($active) || !count($active)) #If no mail is on the server
			{
				...
				$query->run([':user' => $user->id, ':account' => $account]); #Delete unused list of downloaded mails (NOTE : Ignoring error here)
			}
			else
			{
				...
				{
					$target = [];
					$param = [':user' => $user->id, ':account' => $account];
		...
		$query->run([':updated' => gmdate('Y-m-d H:i:s'), ':id' => $folder, ':user' => $user->id]);
