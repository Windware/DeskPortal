$self.conf = new function()
{
	...
	var _default = function(xml, removed) //Default action against configuration saves
	{
		...
		{
			case '0' :
				$system.gui.alert($id, 'user/conf/success/title', 'user/conf/success/message')
				break

			default :
				return $system.gui.alert($id, 'user/conf/error/title', 'user/conf/error/message') && false
				break
		}

		var form = {
			account: $system.node.id($id + '_conf_folder_form_account').account,
			folder : $system.node.id($id + '_conf_folder_form_adjust').source,
		}
	...
	this.change = function(account) //Change to another account on the account edit screen
	{
		...
		for(var i = 0; i < form.elements.length; i++)
		{
			...
			switch(form.elements[i].type)
			{
				...
				case 'checkbox' :
					form.elements[i].checked = value == '1'
					break
	...
	this.create = function(form) //Create a new folder
	{
		...
		return $system.network.send($self.info.root + 'server/php/run.php', {task: 'conf.create'}, {
			account: account,
			parent : base,
			name   : name,
		}, done)
	...
	this.folder = function(account, callback) //Pick the account for folder options
	{
		...
		var update = function(request)
		{
			...
			var form = {
				adjust: $system.node.id($id + '_conf_folder_form_adjust'),
				assign: $system.node.id($id + '_conf_folder_form_assign'),
			}
			...
			var clone = {
				assign: $system.array.list('folder_drafts folder_sent folder_trash'),
				create: ['base'],
				adjust: ['source', 'move'],
			}
	...
	this.move = function() //Move a folder to another folder
	{
		...
		var done = function(request) { _default(request.xml) }
		return $system.network.send($self.info.root + 'server/php/run.php', {task: 'conf.move'}, {
			folder: folder,
			target: target,
		}, done)
	}
	...
	this.remove = function() //Delete a folder
	{
		...
		return $system.network.send($self.info.root + 'server/php/run.php', {task: 'conf.remove'}, {folder: [folder]}, $system.app.method(done, [folder]))
	}
	...
	this.rename = function() //Rename a folder
	{
		...
		var done = function(request) { _default(request.xml) }
		return $system.network.send($self.info.root + 'server/php/run.php', {task: 'conf.rename'}, {
			folder: folder,
			name  : name,
		}, done)
	}
	...
	this.set = function(form) //Apply account configuration changes
	{
		...
		var update = function(request)
		{
			...
			switch($system.dom.status(request.xml))
			{
				...
				case '2' :
					return $system.gui.alert($id, 'user/conf/auth', 'user/conf/read')
					break

				case '3' :
					return $system.gui.alert($id, 'user/conf/auth', 'user/conf/send')
					break

				default :
					return $system.gui.alert($id, 'user/conf/error/title', 'user/conf/error/message')
					break
			}
		...
		return $system.network.send($self.info.root + 'server/php/run.php', {task: 'conf.set'}, option, update)
	}
	...
	this.special = function() //Set the special folders
	{
		...
		var value = {
			account: account,
			drafts : form.folder_drafts.value,
			sent   : form.folder_sent.value,
			trash  : form.folder_trash.value,
		}
		return $system.network.send($self.info.root + 'server/php/run.php', {task: 'conf.special'}, value, done)
	}
	...
	this.subscribe = function() //Subscribe or unsubscribe to a folder
	{
		...
		var done = function(request) { _default(request.xml) }
		return $system.network.send($self.info.root + 'server/php/run.php', {task: 'conf.subscribe'}, {
			folder: folder,
			mode  : mode,
		}, done)
	}
