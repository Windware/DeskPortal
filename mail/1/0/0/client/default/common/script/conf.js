	...
	$self.conf = new function()
	{
		...
		this.create = function(form) //Create a new folder
		{
			...
			return $system.network.send($self.info.root + 'server/php/run.php', {task : 'conf.create'}, {account : account, parent : base, name : name}, done);
		}
		...
		this.folder = function(account, callback) //Pick the account for folder options
		{
			...
			return $system.network.send($self.info.root + 'server/php/run.php', {task : 'conf.folder', account : account, update : 2, subscribed : 0}, null, update);
		}
		...
		this.move = function() //Move a folder to another folder
		{
			...
			var done = function(request) { _default(request.xml); }
			return $system.network.send($self.info.root + 'server/php/run.php', {task : 'conf.move'}, {folder : folder, target : target}, done);
		}
		...
		this.rename = function() //Rename a folder
		{
			...
			var done = function(request) { _default(request.xml); }
			return $system.network.send($self.info.root + 'server/php/run.php', {task : 'conf.rename'}, {folder : folder, name : name}, done);
		}
		...
		this.subscribe = function() //Subscribe or unsubscribe to a folder
		{
			...
			var done = function(request) { _default(request.xml); }
			return $system.network.send($self.info.root + 'server/php/run.php', {task : 'conf.subscribe'}, {folder : folder, mode : mode}, done);
		}
