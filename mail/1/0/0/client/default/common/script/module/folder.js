	...
	$self.folder = new function()
	{
		...
		this.get = function(account, update, callback, request) //List folders for an account
		{
			...
			$self.gui.indicator(true); //Show indicator
			return $system.network.send($self.info.root + 'server/php/run.php', {task : 'folder.get', account : account, update : update, subscribed : 1}, null, $system.app.method(list, [account, update, callback]));
		}
	}
