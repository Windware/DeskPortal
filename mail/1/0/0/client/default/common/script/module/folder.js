$self.folder = new function()
{
	...
	this.change = function(folder, callback) //Change the displaying folder
	{
		...
		if(__selected.folder) //If a previously chosen folder exists
		{
			...
			_scroll[__selected.folder][__page[__selected.folder]] = {
				order   : __selected.order,
				reverse : __selected.reverse,
				position: table.parentNode.scrollTop,
		...
		__selected = {
			account: __selected.account,
			folder : folder,
			marked : form.marked.checked,
			unread : form.unread.checked,
			order  : __order.item,
			reverse: __order.reverse,
	...
	this.empty = function(account) //Empty trash folder
	{
		...
		return $system.network.send($self.info.root + 'server/php/run.php', {task: 'folder.empty'}, {account: account}, $system.app.method(clean, [account, __special.trash[account]]))
	}
	...
	this.get = function(account, update, callback, request) //List folders for an account
	{
			...
			var construct = function(tree, depth) //Create the directory structure tree
			{
				...
				for(var i = 0; i < nodes.length; i++)
				{
					...
					if(special === false) //Create folder icon for regular folders
					{
						...
						$system.image.set(icon, $self.info.devroot + 'image/folder.png')
					...
					if(special !== false) //When it's a special folder
					{
						...
						$system.image.set(icon, $self.info.devroot + 'image/' + special.name + '.png')
						...
						if(special.name == 'trash')
						{
							...
							$system.image.set(icon, $self.info.devroot + 'image/empty.png')
		...
		return $system.network.send($self.info.root + 'server/php/run.php', {
			...
			update    : update,
			subscribed: 1,
		}, null, $system.app.method(list, [account, update, callback]))
	}
}
