	...
	$self.item = new function()
	{
		...
		this.mark = function(id, mode) //Flag a mail as marked or not
		{
			...
			return $system.network.send($self.info.root + 'server/php/run.php', {task : 'item.mark'}, {id : id, mode : mode ? 1 : 0}, notify);
		}
		...
		this.move = function(id, folder, callback) //Move the mails to another folder
		{
			...
			return $system.network.send($self.info.root + 'server/php/run.php', {task : 'item.move'}, {id : $system.array.unique(id), folder : folder}, $system.app.method(notify, [folder, reload, callback]));
		}
