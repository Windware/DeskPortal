$self.account = new function()
{
	...
	this.get = function(cache, list, callback) //Get list of accounts
	{
		...
		var list = function(request)
		{
			...
			var conf = {
				account: $system.node.id($id + '_conf_form'),
				folder : $system.node.id($id + '_conf_folder_form_account'),
			} //Configuration panel forms
		...
		return $system.network.send($self.info.root + 'server/php/run.php', {task: 'account.get'}, null, list)
	}
	...
	this.remove = function(id) //Remove an account
	{
		...
		var notify = function(id, request)
		{
			switch($system.dom.status(request.xml))
			{
				...
				default :
					$system.gui.alert($id, 'user/conf/error/title', 'user/conf/error/remove')
					break
			}
		}

		return $system.network.send($self.info.root + 'server/php/run.php', {task: 'account.remove'}, {id: id}, $system.app.method(notify, [id]))
	}
}
