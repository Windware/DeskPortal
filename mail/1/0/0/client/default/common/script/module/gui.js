	...
	$self.gui = new function()
	{
		...
		var _body = function(id, index, compose, field) //Create the mail window content
		{
			...
			else //For composing the mail
			{
				...
				var value = {id : id, index : index, action : $system.network.form($self.info.root + 'server/php/run.php?task=gui.send')};
		...
		var _insert = function(id, index, raw) //Load the message body for composing
		{
			...
			return $system.network.send($self.info.root + 'server/php/run.php', {task : 'gui.compose', id : id}, null, $system.app.method(load, [id, index]));
		}
