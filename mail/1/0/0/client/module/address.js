	...
	$self.address = new function()
	{
		...
		this.list = function(index) //Load the addresses
		{
			...
			return $system.network.send($self.info.root + 'server/php/front.php', {task : 'address.list', group : form.group.value, type : form.type.value}, null, $system.app.method(display, [form.type.value, index]));
		}
		...
		this.open = function(index) //Load registered addresses in the addressbook app
		{
			...
			var group = function(index)
			{
				...
				return $system.network.send($self.info.root + 'server/php/front.php', {task : 'address.open'}, null, $system.app.method(display, [index]));
			}
