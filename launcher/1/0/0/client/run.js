	...
	$self.run = function(callback)
	{
		...
		var display = function(callback, request) //Get and set the list of applications
		{
			...
			for(var i = 0; i < list.length; i++)
			{
				...
				for(var j = 0; j < entries.length; j++)
				{
					...
					if(!remove) section += $system.text.format($self.info.template.list, {icon : icon, app : name, display : view});
				}
		...
		return $system.network.send($self.info.root + 'server/php/front.php', {task : 'run', language : $global.user.language}, null, $system.app.method(display, [callback]));
	}
