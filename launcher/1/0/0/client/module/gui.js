
	$self.gui = new function()
	{
		var _class = $id + '.gui';

		var _exclude = ['launcher', 'login', 'system']; //List of application to exclude from listing

		this.names = {}; //Category display names

		this.expand = function(category) //Show or hide a category
		{
			var region = $id + '_list_' + category; //The category app area
			if(!$system.node.id(region)) return false;

			var hidden = $system.node.hidden(region);
			$system.tip.set([$id, 'category', category].join('_'), $id, hidden ? 'close' : 'open', [$self.gui.names[category]]);

			$system.node.fade(region); //Open up the area
			return $system.network.send($self.info.root + 'server/php/front.php', {task : 'gui.expand'}, {category : category, state : hidden ? 1 : 0}); //Save the state
		}

		this.launch = function(id) { return $system.node.id(id) ? $system.tool.fade(id) : $system.tool.create(id); } //Load or close an application

		this.list = function(callback) //Get and set the list of applications
		{
			var display = function(callback, request)
			{
				var list = $system.dom.tags(request.xml, 'category');

				if(!list.length)
				{
					var log = $system.log.init(_class + '.list');

					if(typeof callback == 'function') callback();
					return log.dev($global.log.warning, 'dev/list', 'dev/list/solution');
				}

				var language = $system.language.strings($id);
				var launcher = ''; //Application list in HTML

				var expand = []; //List of expanded categories

				for(var i = 0; i < list.length; i++)
				{
					var category = $system.dom.attribute(list[i], 'name');
					var show = $system.dom.attribute(list[i], 'display');

					var entries = $system.dom.tags(list[i], 'entry');
					var section = ''; //This category's entries

					for(var j = 0; j < entries.length; j++)
					{
						var name = $system.dom.attribute(entries[j], 'name');
						var view = $global.app[name] && $global.app[name].title || name;

						if(!$system.is.id(name) || view == '') continue;
						if($system.array.find(_exclude, name.split('_', 2)[0])) continue; //Remove excluded items

						if($system.dom.attribute(entries[j], 'icon')) //Make icon if it exists
						{
							var graphic = $system.image.source(name, 'graphic/icon.png', $system.dom.attribute(entries[j], 'icon'));
							var icon = '<img class="' + $system.info.id + '_icon" src="' + graphic + '" /> ';
						}
						else var icon = '';

						section += $system.text.format($self.info.template.list, {icon : icon, app : name, display : view});
					}

					if(section) //If entries are found, create the category
					{
						var values = {category : category, section : show, entries : section};
						launcher += $system.text.format($self.info.template.header, values);

						$self.gui.names[category] = show;
						if($system.dom.attribute(list[i], 'expand') === '1') expand.push(category); //Remember which categories to expand
					}
				}

				$system.node.id($id + '_box').innerHTML = launcher;

				for(var name in $self.gui.names) //Give tip to the category links
				{
					var link = [$id, 'category', name].join('_');

					$system.tip.set(link, $id, 'open', [$self.gui.names[name]]);
					$system.node.hover(link, $id + '_active'); //Make hover style
				}

				for(var i = 0; i < expand.length; i++) $system.node.hide($id + '_list_' + expand[i], false); //Expand the categories
				if(typeof callback == 'function') callback();
			}

			return $system.network.send($self.info.root + 'server/php/front.php', {task : 'gui.list', language : $global.user.language}, null, $system.app.method(display, [callback]));
		}

		this.logout = function() //Logs the user out
		{
			var language = $system.language.strings($id);
			if(confirm(language.confirm)) $system.user.logout(true);
		}
	}
