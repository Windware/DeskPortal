	...
	$self.user = new function() //User related class
	{
		...
		this.init = function() //Initialize user variables
		{
			...
			$global.user.pref.format = {full : '%Y% %M% %d% %h%:%N%', date : '%Y% %M% %d%', year : '%Y%', month : '%Y% %M%', monthdate : '%M% %d%', time : '%h%:%N%'}; //Date display format
		...
		this.conf = function(app) //Loads the user's application configuration
		{
			...
			var request = $system.network.send($system.info.root + 'server/php/front.php', query, null, null); //Get it from the server
		...
		this.info = function() //Loads the user's preferences
		{
			...
			var param = {task : 'user.info', language : location.search.replace(/\?/, '')};

			var request = $system.network.send($system.info.root + 'server/php/front.php', param, null, null);
			...
			if($system.dom.status(request.xml) != '0') //If a failed status was sent
			{
				...
				$system.browser.cookie('name', '', true); //Let go of the name cookie
		...
		this.refresh = function() //Refresh the ticket (TODO : How to exchange ticket to a new value without breaking?)
		{
			...
			return $system.network.send($system.info.root + 'server/php/front.php', {task : 'user.refresh'}, null, validate);
		}
