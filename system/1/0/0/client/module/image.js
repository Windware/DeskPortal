	...
	$self.image = new function() //Image related class
	{
		...
		this.set = function(node, image) //Sets an image element's source
		{
			...
			{
				node.src = $system.network.form($system.info.devroot + 'graphic/blank.png'); //Give a blank image
		...
		this.wallpaper = function(name, save) //Set a wallpaper
		{
			...
			if(!save) return true;
			return $system.network.send($system.info.root + 'server/php/front.php', {task : 'image.wallpaper'}, {name : name});
		}
	}
