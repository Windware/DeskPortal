$self.node = new function() //Node manipulation class
{
	...
	this.remove = function(node) //Remove a node
	{
		...
		try
		{ if(target.parentNode.removeChild(target)) return true } //Remove the node from the parent object

		catch(error)
		{ return log.dev($global.log.error, 'dev/remove/fail', 'dev/check', [$system.browser.report(error)]) }
	}
	...
	this.target = function(subject) //Finds the node by either the node reference itself or the name of its ID
	{
		switch(typeof subject)
		{
			case 'object' :
				return subject
				break //If an object is passed, return as is

			case 'string' :
				return $system.node.id(subject)
				break //If a string is passed, assume it's a DOM id

			default :
				return null
				break //Otherwise, return a null object
		}
	}
