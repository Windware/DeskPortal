$self.app = new function() //Application related class
{
	...
	this.callback = function(source, callback, param) //Run a callback function safely
	{
		...
		try
		{ return $system.is.array(param) ? $system.app.method(callback, param)() : callback() }
	...
	this.conf = function(id, pair) //Sets the application configuration value
	{
		...
		return $system.network.send($system.info.root + 'server/php/run.php', {task: 'app.conf', id: id}, pair)
	}
	...
	this.library = function(id) //Load the application library
	{
		...
		$private.info = $system.app.path(id) + 'client/default/common/script/info.js' //Get the application information
		...
		new function(_class, log, id) //Run in a closed environment to allow $id/$self/$system re-declaration inside
		{
			...
			if(!$system.is.object($global.user.conf[$id])) $global.user.conf[$id] = {} //Create application preference hash
			...
			with($self) //Define the application information
			{
				...
				info.devroot = $global.user.conf[$id] && $global.user.conf[$id].theme ? $global.user.conf[$id].theme : info.root + 'client/default/common/'
			}

			try
			{ with($self.info) eval($private.request.text) } //Load the info script with its own 'info' name space applied

			catch(error)
			{ return $private.failure('dev/info', 'dev/check', [$id, $system.browser.report(error)]) }
			...
			try
			{ with($self.info) eval($private.request.text) } //Load the style script with its own 'info' name space applied

			catch(error)
			{ return $private.failure('dev/style', 'dev/check', [$id, $system.browser.report(error)]) }
			...
			var request = $system.network.item($self.info.root + 'client/default/common/script/module/', true)

			var load = $system.array.list('conf manual run init') //Add required scripts for loading
			for(var i = 0; i < load.length; i++) request.push($system.network.item($self.info.root + 'client/default/common/script/' + load[i] + '.js'))
			...
			try
			{ eval($private.code) } //Run the codes

			catch(error)
			{ return $private.failure('dev/library/init', 'dev/library/init/solution', [$id, $system.browser.report(error)]) }
	...
	this.load = function(id, execute, front, quick) //Load and run an application : TODO - Make it less synchronous to avoid GUI interface slow down
	{
		...
		$private.run = function() //Run from setTimeout to avoid locking quitting the function
		{
			...
			$private.ready = function($private, execute, front, quick, id, log, info) //Run the application code just before the window goes live
			{
				...
				if(typeof $self.run !== 'function') //If initial execute code does not exist
				{
					...
					{
						try
						{ $private.execute() } //Run the callback directly
				...
				try
				{ $self.run($private.execute) } //Run the initial execution code queueing the callback
		...
		setTimeout($private.run, 0) //Quit this function first by queuing the later operations
	}
	...
	this.prepare = function(id, core) //Preload the initial files
	{
		...
		if(!core) load.push(info.root + 'client/default/common/script/*.js', info.root + 'client/default/common/script/module/*.js') //Add every scripts
		...
		load.push(info.root + 'meta.xml', info.devroot + 'image/icon.png')
		...
		for(var i = 0; i < info.depend.length; i++) //Load any application's info file that the application depends on
			if(!$global.top[info.depend[i]]) load.push($system.app.path(info.depend[i]) + 'client/default/common/script/info.js')
	...
	this.unload = function(id, execute) //Unload the application and all applications depending on it
	{
		...
		$private.remove = function(id, _class, execute, log, system) //Remove the application
		{
			...
			$private.request = $private.system.network.item($private.system.info.root + 'client/default/common/script/unload.js') //Load the unload script from cache
			i = undefined //Clear out the local variable before the 'eval'

			if($private.request.valid()) //If the unloading script exists
			{
				try
				{ eval($private.request.text) } //Run the unloading script
