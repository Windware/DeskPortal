$self.window = new function() //Window object manipulation class
{
	...
	var _window = function(id) //An object to track the window's condition
	{
		...
		this.visible = function(part) //Return the visibility of various parts of a window
		{
			switch(part)
			{
				case 'body' :
				case 'bar' :
					return this[part] && !$system.node.hidden(this[part].id)
					break

				default :
					return this.object && !$system.node.hidden(this.object.id)
					break
			}
		}
	}
	...
	this.init = function() //Initialize window templates
	{
		...
		for(var i = 0; i < request.length; i++) //Process templates
		{
			if(!request[i].valid(true)) //Quit with critical error, if they are unavailable
				return log.dev($global.log.critical, 'dev/window/template', 'dev/window/template/solution', [request[i].file])
	...
	{ //TODO - Add hover interface. Make it switchable in user configuration. Put mouse over an action link for configured amount of time (ex : 3sec) and make the action occur
		...
		{
			var request = $system.network.item($global.top[id].info.devroot + 'image/icon.png') //Get the icon request object
		...
		pane.design = $system.text.format('%%server/php/run.php?task=window.create&color=%%&background=%%&border=%%&edge=%%&shadow=%%&round=%%&place=', replacer)
		...
		{
			try
			{ prepare() } //Do any preparation sequence before the window goes live
	...
	this.save = function(id, parameter) //Saves the window configuration
	{
		...
		$system.network.send($system.info.root + 'server/php/run.php', {
			task   : 'window.save',
			section: 'window',
		}, parameter, $system.gui.check)
	}
