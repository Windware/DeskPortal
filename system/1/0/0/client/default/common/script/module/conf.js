	...
	$self.conf = new function() //Configuration panel related
	{
		...
		this.apply = function(id, form) //Apply change to the general configuration panel
		{
			...
			$system.network.send($system.info.root + 'server/php/run.php', {task : 'conf.apply'}, {name : id.replace(/(_\d+){3}$/, ''), version : form.version.value}, notify);
		...
		this.swap = function(id, panel) //Swap the information window content
		{
			...
			switch(panel)
			{
				...
				case 'history' : //Load the version history
					...
					{
						var request = $system.network.send($system.info.root + 'server/php/run.php', {task : 'conf.swap', app : app[0]}, null, null); //TODO - Should be asynchronized
