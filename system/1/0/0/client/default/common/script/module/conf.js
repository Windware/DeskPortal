$self.conf = new function() //Configuration panel related
{
	...
	this.apply = function(id, form) //Apply change to the general configuration panel
	{
		...
		$system.network.send($system.info.root + 'server/php/run.php', {task: 'conf.apply'}, {
			name   : id.replace(/(_\d+){3}$/, ''),
	...
	this.change = function(id, section) //Switch to another configuration tab
	{
		...
		{
			try
			{ $global.top[id].conf[section]() } //Run the custom configuration code

			catch(error)
			{ log.dev($global.log.error, 'dev/tool/script', 'dev/check', [id, section, 'configuration', $system.browser.report(error)]) }
	...
	this.flip = function(id, section, page) //Flip a page of a manual
	{
		...
		{
			try
			{ $global.top[id].manual[page]() } //Run the custom script for the manual page

			catch(error)
			{ log.dev($global.log.error, 'dev/tool/script', 'dev/check', [id, page, 'manual', $system.browser.report(error)]) }
		}
	...
	this.swap = function(id, panel) //Swap the information window content
	{
		...
		if(_previous[id])
		{
			...
			$system.image.set(image, $system.info.devroot + 'image/' + image.name + '_grey.png')
		}
		...
		var image = $system.node.id(_previous[id].tab).firstChild
		$system.image.set(image, $system.info.devroot + 'image/' + image.name + '.png')
		...
		switch(panel)
		{
			case 'about' : //Build up the app's information from source files
				var replace = function(phrase, match)
				{
					switch(match)
					{
						...
						case 'version' :
							return info.version.replace(/_/g, '.')
							break //Application version
					}
				}
			...
			case 'conf' : //Configuration panel
				...
				for(var i = 0; i < request.length; i++) //For all of the configuration templates
				{
					...
					tabs += $system.text.format($system.text.template(html, id), {
						section: name,
						length : length,
			...
			case 'history' : //Load the version history
				...
				{
					var request = $system.network.send($system.info.root + 'server/php/run.php', {
						task: 'conf.swap',
						app : app[0],
					}, null, null) //TODO - Should be asynchronized
			...
			default :
				return false
				break //Quit as error for wrong pages
		}
