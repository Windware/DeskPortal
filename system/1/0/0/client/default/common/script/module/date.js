$self.date = new function() //Date manipulation class
{
	...
	var _date = function(date) //Object to initialize for date manipulation
	{
		...
		this.format = function(template) //Return the user configured date format string
		{
			...
			var replace = function(phrase, match) //Replace the template variables into the date object's value
			{
				...
				{
					case 'Y' :
						return locale.year.replace('%%', _self.year())
						break

					case 'm' :
						return locale.month.replace('%%', _self.month())
						break

					case 'M' :
						return locale.month.replace('%%', _self.month('full'))
						break

					case 'd' :
						return locale.day.replace('%%', _self.date())
						break

					case 'w' :
						return _self.day('less')
						break

					case 'W' :
						return _self.day('full')
						break

					case 't' :
						return _self.hour() < 12 ? local.am : locale.pm
						break

					case 'h' :
						return locale.hour.replace('%%', _self.hour())
						break

					case 'H' :
						return locale.hour.replace('%%', _self.hour(true))
						break

					case 'i' :
						return locale.hour.replace('%%', _self.hour(false, true))
						break

					case 'I' :
						return locale.hour.replace('%%', _self.hour(true, true))
						break

					case 'n' :
						return locale.minute.replace('%%', _self.minute())
						break

					case 'N' :
						return locale.minute.replace('%%', _self.minute(true))
						break
	...
	this.create = function(source, invalid) //Create the date object from a time source
	{
		...
		else if($system.is.array(source)) //If an array of time is given
		{
			if($system.is.digit(source[0])) //At least year is required
			{
				...
				if(!invalid) //If not allowing invalid dates, check for the date's validity by matching the date values to the value specified
				{
	...
	this.select = function(start, end, descend, selected, pad, step) //Creates 'option' nodes to be appended on 'select' form
	{
