$self.browser = new function() //Browser specific processing
{
	...
	this.init = function() //Detect the browser type
	{
		...
		{
			if(browser[i].userAgent)
			{ if(!navigator.userAgent.match(browser[i].userAgent)) continue }
			else if(browser[i].vendor)
			{ if(!navigator.vendor || !navigator.vendor.match(browser[i].vendor)) continue }
			else if(browser[i].opera)
		...
		{
			if(os[i].platform)
			{ if(!navigator.platform.match(os[i].platform)) continue }
			else continue
		...
		if(dev.engine == 'trident' || (dev.engine == 'webkit' && dev.version < 3)) dev.click = {
			left  : 1,
			middle: 0,
			right : 2,
		}
		...
		{
			try
			{ dev.request = function() { return new ActiveXObject('Msxml2.XMLHTTP') } } //Try newer version first

			catch(error) //On failure
			{
				try
				{ dev.request = function() { return new ActiveXObject('Microsoft.XMLHTTP') } } //Try an older version

				catch(error)
				{ } //Give up otherwise (Errors are handled commonly below)
			}
		}
		...
		for(var i = 0; i < dev.language.length; i++) if(!$system.is.language(dev.language[i])) dev.language.splice(i, 1)
	...
	this.size = function() //Returns the size of the browser window
	{
		...
		{ return log.dev($global.log.error, 'dev/size', 'dev/size/solution', null, null, {}) }
	}
}
