$self.motion = new function() //Window dragging class
{
	...
	var _static = ['A', 'IMG', 'INPUT', 'LABEL', 'SELECT', 'OPTION', 'TEXTAREA'] //Nodes that should not trigger dragging windows
	...
	this.init = function() //Initialize the move box object's background image
	{
		...
		var color = {move: '000000', resize: 'ffeeaa'} //Colors for move box object when moving and resizing (TODO : Should be in stylesheet)

		for(var type in color)
		{
			var params = '%%server/php/run.php?task=motion.init&color=333333&background=%%&border=333333&round=%%&shadow=%%&place=circle&edge=%%' //TODO : Should be in stylesheet
	...
	this.move = function(event) //Moves the window by mouse dragging (Have as less computation as possible)
	{
		...
		else
		{
			...
			if($system.browser.engine === 'trident') event = $system.event.source(arguments) //For IE, event passed as parameter returns different button value...
			else if(!event) event = window.event //NOTE : Not using 'event.source' for a possible performance reason for other browsers

			//If mouse is not held, quit moving it (Happens when the cursor goes out of browser space before the button goes up) (khtml reports 65535 on event.button, IE9 reports different value on button in 'onmousedown' and 'onmousemove' event)
	...
	this.stop = function() //Stop any windows from moving
	{
		...
		{
			$system.node.opacity(_mover.node.id, 100) //Set back to normal opacity
			if($system.browser.engine === 'trident') _mover.node.style.removeAttribute('filter') //Avoid corrupted style under IE
		}
		...
		if(_mover.diff && $system.is.element(_mover.node)) //If moving or resizing took place and the original object still exists
		{
			if(_mover.resize) //If resizing
			{
				switch(_mover.type)
				{
					...
					case _type.clone :
					case _type.box : //Calculate the final size by the amount moved
			...
			else //If moving
			{
				...
				{
					case _type.self :
					case _type.clone : //If not box type, use the object's position
