...
switch($_GET['task'])
{
	...
	case 'conf.apply' : //Apply general configuration tab changes
		...
		else print $system->xml_dump($user->save('version', ['name' => $_POST['name'], 'version' => $_POST['version']]));
	break;
	...
	case 'image.wallpaper' : //Save user wallpaper
		...
		if(!$user->valid) print $system->xml_dump(-1);
		else print $system->xml_dump($user->save('conf', ['wallpaper' => $_POST['name']], 'system_static'));
	break;

	case 'motion.init' :
	case 'tip.make' :
	case 'window.create' : //Create window background image
	...
	case 'tool.create' :
	case 'tool.fade' :
	case 'window.save' : //Save app states
		$user = $system->user();
	...
	case 'user.conf' : //Get user app confs
		...
		{
			$list = is_array($_GET['app']) ? $_GET['app'] : [];
