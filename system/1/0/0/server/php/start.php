<?php
	...
	#Create a cached JavaScript from a list of system core JavaScripts and output them
	$code = $system->cache_get('library.js', $system->compress_header());
	...
	if(!$code) #If the JavaScript cache cannot be retrieved, create it
	{
		...
		$static .= $system->file_read("{$system->system['root']}client/start.js", LOG_CRIT)."\n";
		$static = str_replace($source, $replacer, $static); #Replace the variable placeholders

		$stored = "{$system->self['root']}client/"; #Start building a concatenated script to cache
