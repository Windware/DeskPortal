...
class System_1_0_0_Network
{
	...
	private static $_content = []; //Data retrieved when max size is limited

	private static $_exceed = []; //Flag to note if a stream response exceeded maximum size specified

	private static $_relation = []; //Data relation to streams
	...
	public static function http(&$system, $request) //Make multiple remote HTTP requests in parallel (TODO - HTTP authentication is not implemented as of now)
	{
		...
		$bundle = $content = []; //List of connections and result contents
		$pool = curl_multi_init(); //Initialize the multiple connection pool //TODO - Check for curl module

		$conf = $system->app_conf('system', 'static'); //System configuration values

		//Basic curl options to set
		$options = [CURLOPT_AUTOREFERER       => true, //Send referrer page against redirects
		...
		foreach($request as $info) //For all of the page requests
		{
			...
			foreach($info['header'] as $key => $request) //Set given headers
			{
				...
				{
					case 'Cookie' :
						curl_setopt($channel, CURLOPT_COOKIE, $request);
					break; //TODO - Point is?

					case 'If-Modified-Since' :
						curl_setopt($channel, CURLOPT_TIMEVALUE, strtotime($request));
					break; //TODO - Point is?

					case 'Referer' :
						curl_setopt($channel, CURLOPT_REFERER, $request);
					break; //TODO - Point is?

					default :
						curl_setopt($channel, CURLOPT_HTTPHEADER, ["$key: $request"]);
					break;
				}
			}
			...
			switch(@$info['method'])
			{
				...
				break; //Go with HEAD request if specified
			...
			if($limit[$id] = $system->is_digit(@$info['max'])) //If limiting
			{
				...
				curl_setopt($channel, CURLOPT_WRITEFUNCTION, [__CLASS__, '_write']); //Pipe the response through a callback to limit the retieval size
			}
		...
		do
		{
			$task = curl_multi_exec($pool, $active);
		}
		while($task == CURLM_CALL_MULTI_PERFORM);
		...
		foreach($bundle as $address => $channel) //For all the connections
		{
			...
			$content[] = ['address' => $address, 'status' => $status, 'header' => $header, 'body' => @self::$_exceed[$id] ? '' : $body, 'exceed' => !!@self::$_exceed[$id]];
		}
