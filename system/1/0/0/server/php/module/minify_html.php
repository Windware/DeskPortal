...
class System_1_0_0_Minify_Html
{
	...
	public function process()
	{
		...
		$this->_html = preg_replace_callback('/(\\s*)(<script\\b[^>]*?>)([\\s\\S]*?)<\\/script>(\\s*)/i', [$this, '_removeScriptCB'], $this->_html);

		//Replace STYLEs (and minify) with placeholders
		$this->_html = preg_replace_callback('/\\s*(<style\\b[^>]*?>)([\\s\\S]*?)<\\/style>\\s*/i', [$this, '_removeStyleCB'], $this->_html);

		//Remove HTML comments (not containing IE conditional comments).
		$this->_html = preg_replace_callback('/<!--([\\s\\S]*?)-->/', [$this, '_commentCB'], $this->_html);

		//Replace PREs with placeholders
		$this->_html = preg_replace_callback('/\\s*(<pre\\b[^>]*?>[\\s\\S]*?<\\/pre>)\\s*/i', [$this, '_removePreCB'], $this->_html);

		//Replace TEXTAREAs with placeholders
		$this->_html = preg_replace_callback('/\\s*(<textarea\\b[^>]*?>[\\s\\S]*?<\\/textarea>)\\s*/i', [$this, '_removeTextareaCB'], $this->_html);
		...
		$this->_html = preg_replace_callback('/>([^<]+)</', [$this, '_outsideTagCB'], $this->_html);
