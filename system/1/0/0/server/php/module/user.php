<?php
	class System_1_0_0_User
	{
		...
		public function __construct(&$system, $id = null) //Get the user info (If no user is specified, gets the logged in user's)
		{
			...
			if($id === null) //For logged in user
			{
				if($conf['system_demo']) //For demo mode
				{
					...
					$parameters = array(':identity' => $_COOKIE['identity']); //Only check the user name to allow multiple client accesses
				}
				else
				{
					//Query to be used
					...
					$parameters = array(':ticket' => $_COOKIE['ticket'], ':ip' => $_SERVER['REMOTE_ADDR'], ':identity' => $_COOKIE['identity']);
				}
		...
		public static function create(&$system, $identity, $password) //Create a new user
		{
			...
			$query->run(array(':identity' => $identity, ':password' => hash($conf['user_hash'], $password), ':joined' => gmdate('Y-m-d')));
		...
		public static function find(&$system, $identity) //Find user ID from name
		{
			...
			$query->run(array(':identity' => $identity));
