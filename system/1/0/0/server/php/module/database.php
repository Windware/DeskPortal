...
class System_1_0_0_Database
{
	...
	protected function _create() //To check and create application database tables
	{
		...
		{
		} //If the table does not exist, continue on //FIXME - When other error occurs, it still tries to create new tables
	...
	public function __construct(&$system, $type, $caller, System_1_0_0_User $user = null, $app, $version)
	{
		...
		$database = str_replace(['%APP%', '%VERSION%'], [$app, $version], $database);
		...
		{
			$this->handler = new PDO($method, $name, $pass, [PDO::ATTR_PERSISTENT => !!$conf["db_{$type}_persistent"]]);
	...
	{
		if($this->locked && $this->success) return $this->rollback();
	} //Rollback changes under locked mode
	...
	public function begin() //Start a transaction
	{
		...
		{
			return $this->logger(LOG_ERR, 'Cannot start a transaction : ' . $error->getMessage(), 'Check the error');
		}
	}
	...
	public function commit() //Commit the changes atomically
	{
		...
		{
			return $this->logger(LOG_ERR, 'Cannot commit changes of a transaction : ' . $error->getMessage(), 'Check the error');
		}
	}
	...
	{
		return is_string($string) ? str_replace([self::$character, '%', '_'], [self::$character . self::$character, self::$character . '%', self::$character . '_'], $string) : '';
	}
	...
	public static function key(&$system, $type, $key, $value = null, System_1_0_0_User $user = null, $app = null, $version = null)
	{
		...
		$data = [':name' => $key]; //Data for the query
	...
	public function id()
	{
		return $this->success ? $this->handler->lastInsertId() : null;
	...
	public function rollback() //Rollback the transaction
	{
		...
		{
			return $this->logger(LOG_ERR, 'Cannot rollback changes of a transaction : ' . $error->getMessage(), 'Check the error');
		}
	}

	//Make a prepared statement (Second parameter is only used from the logging module to avoid looping)
	public function prepare($query, $level = LOG_EMERG)
	{
		return new System_1_0_0_Database_Query($this, $this->_system, $query, $level);
	}
	...
	public function schema($schema) //Read database schema file and parse its XML
	{
		...
		try
		{
			return new SimpleXMLElement($system->file_read($system->global['define']['top'] . $schema));
		}
...
class System_1_0_0_Database_Query //An object to be built for each queries
{
	...
	public function __construct(&$database, &$system, $query, $level)
	{
		...
		try
		{
			$this->handler = $database->handler->prepare($query);
	...
	public function run($parameter = []) //Executes the prepared statement
	{
