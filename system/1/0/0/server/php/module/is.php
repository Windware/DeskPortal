...
class System_1_0_0_Is //Check variables
{
	...
	{
		return is_string($color) && preg_match('/^[0-9a-f]{6}$/', $color);
	} //Finds if it is a color code

	public static function digit($subject, $decimal = false, $signed = false) //Returns if the given string is a valid numerical representation
	{
	...
	public static function language($subject)
	{
		return !!preg_match('/^[a-z]{2}(-[a-z]{2})?$/i', $subject);
	}

	public static function mail($address)
	{
		return !!preg_match('/.@./', $address);
	} //Find out if the given string is a mail address or not, in a simple way
	...
	public static function ticket($subject)
	{
		return is_string($subject) && preg_match('/^[a-f\d]{32}$/', $subject);
	}

	//Returns if the string is in a DATE or DATETIME format
	public static function time($subject)
	{
		return !!preg_match('/^\d{4}-\d{1,2}-\d{1,2}( \d{1,2}:\d{1,2}:\d{1,2})?$/', $subject);
	}

	//Finds if the given subject is a type specified or undefined
	public static function type($subject, $type)
	{
		return !isset($subject) || gettype($subject) === $type;
	}
}
