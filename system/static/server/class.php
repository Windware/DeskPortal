<?php
	function __autoload($class) #Automatically load the class files when necessary
	{
		...
		if(!$matches[3]) $class = 'server/php/init.php'; #If base class for the application was called
	...
	class System_Static
	{
		...
		public static function app_env($path) #Returns the application environment from a given path
		{
			...
			$env = array(); #Application environment information
		...
		public static function auth_verify($user, $pass)
		{
			...
			return System_Static_Auth::verify($user, $pass); #Run the authorization check
		}
		...
		public static function file_conf($xml) #Read XML config file and return a hash
		{
			...
			$settings = array(); #Settings retrieved
		...
		public static function file_readable($file) { return is_string($file) && is_readable($file) && is_file($file); }
		...
		public static function is_user($subject) { return !!preg_match('/^\w+$/', $subject); }
		...
		public static function system_strip($target = null) #Remove magic quotes
		{
			...
			if(is_array($target)) return array_map(array('self', 'system_strip'), $target);
	...
	class System_Static_Log #Logging class
	{
		...
		public function system($level, $message, $solution = '', $value = false, $section = 'system', $written = false)
		{
			...
			#The log components
			$component = array($count, $global['define']['log'][$level]."($level)", $section, $this->_reporter, $this->_caller, $folder, $message, $solution);
			...
			if($report['display']) #If configured level is higher to display logs
			{
				...
				{
					$severity = array('ccc', '', 'f85', 'f95', 'cba', 'abc', 'fff', 'eee'); #Colorize the levels
