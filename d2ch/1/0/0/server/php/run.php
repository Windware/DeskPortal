...
switch($_GET['task'])
{
	case 'message.get' : #Get messages for a thread
		...
		foreach($list as $row)
		{
			...
			$xml .= $system->xml_node('message', $row, $system->xml_data($message), ['thread']);
		}
	...
	case 'run' : #Get board list
		...
		foreach($category as $id => $name)
		{
			...
			foreach($list[$id] as $entry) $include .= $system->xml_node('board', ['id' => $entry['id'], 'name' => $entry['name']]);

			$xml .= $system->xml_node('category', ['id' => $id, 'name' => $name], $include);
		}
	...
	case 'thread.get' : #Get thread list
		...
		print $system->xml_dump($list !== false, 'thread', $list, ['board', 'file', 'number', 'updated']);
	break;
}
?>
