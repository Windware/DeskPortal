<?php
	class D2ch_1_0_0_Message
	{
		...
		public static function get($thread) #get the message listing of a thread
		{
			...
			if(!$query->run(array(':id' => $thread))) return false;
			...
			if(!$query->run(array(':thread' => $thread))) return false;
		...
		public static function update($thread) #Update the message listing of a thread
		{
			...
			$source = $system->network_http(array(array('address' => "{$row['address']}/dat/{$row['file']}", 'max' => self::$_max * 1000))); #Get subject list
			...
			foreach(array_slice(explode("\n", $source[0]['body']), $index) as $line) #Pick the non recorded messages
			{
				...
				$insert = array(':thread' => $thread, ':number' => ++$index, ':name' => $name, ':mail' => $mail, ':signature' => $signature, ':posted' => $time, ':message' => $message);
			...
			if(!$query->run(array(':updated' => $system->date_datetime(), ':id' => $thread))) return $database->rollback && false;
