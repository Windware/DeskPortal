...
class D2ch_1_0_0_Board
{
	...
	public static function update() #Update list of boards
	{
		...
		$source = $system->network_http([['address' => self::$_source, 'max' => self::$_max * 1000]]);
		if($source[0]['status'] != 200) return false;

		$collection = []; #List of boards found
		...
		foreach(explode("\n", mb_convert_encoding($source[0]['body'], 'utf-8', mb_detect_encoding($source[0]['body']))) as $line)
		{
			...
			elseif($category && preg_match('|<A HREF=(.+?)>(.+?)</A>|i', $line, $match)) $collection[$category][] = ['name' => $match[2], 'address' => str_replace('"', '', $match[1])];
		}

		$query = $database->prepare("SELECT * FROM {$database->prefix}category");
		if(!$query->run()) return false;

		$relation = []; #Category name and ID relation
		...
		foreach(array_diff(array_keys($relation), array_keys($collection)) as $category) if(!$query->run([':id' => $relation[$category]])) return false;
		...
		{
			if(!$query->run([':name' => $category])) return $database->rollback() && false;
		...
		foreach($collection as $category => $list) #For all the categories of boards fetched remotely
		{
			foreach($list as $board) #For each board
			{
				...
				$success = $query->run([':name' => $board['name'], ':address' => $board['address'], ':category' => $relation[$category], ':updated' => null]);
