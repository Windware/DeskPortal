...
class D2ch_1_0_0_Thread
{
	...
	public static function get($board) #Get list of threads
	{
		...
		if(!$query->run([':id' => $board])) return false;
		...
		if(!$query->run([':board' => $board])) return false;
	...
	public static function update($board) #Update list of threads
	{
		...
		$source = $system->network_http([['address' => $query->column() . '/subject.txt', 'max' => self::$_max * 1000]]); #Get subject list
		...
		foreach(explode("\n", $source[0]['body']) as $line)
		{
			...
			if(!$query->run([':board' => $board, ':file' => $item, ':number' => ++$index, ':reply' => $reply, ':title' => $title, ':updated' => $updated[$item]])) return $database->rollback() && false;
		}

		$query = $database->prepare("UPDATE {$database->prefix}board SET updated = :updated WHERE id = :id");
		if(!$query->run([':updated' => $system->date_datetime(), ':id' => $board])) return $database->rollback && false;
