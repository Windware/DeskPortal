	...
	this.get = function(id) //List the messages in a thread
	{
		...
		var list = function(request)
		{
			...
			for(var x = 0; x < message.length; x++)
			{
				...
				__message[id] = {
					id       : id,
					message  : $system.text.link(text.replace(/([^h])ttp:\/\//g, '$1http://')),
					filter   : '',
					name     : $system.dom.attribute(message[x], 'name'),
					mail     : $system.dom.attribute(message[x], 'mail'),
					signature: $system.dom.attribute(message[x], 'signature'),
				}
