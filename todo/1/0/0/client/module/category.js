	...
	$self.category = new function()
	{
		...
		this.remove = function() //Removes the chosen category
		{
			...
			return $system.network.send($self.info.root + 'server/php/front.php', {task : 'category.remove'}, {category : select.value}, removal);
		}
		...
		this.set = function(form) //Set a category
		{
			...
			$system.network.send($self.info.root + 'server/php/front.php', {task : 'category.set'}, values, set);
