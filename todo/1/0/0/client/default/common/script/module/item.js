$self.item = new function()
{
	...
	this.add = function(form) //Add a new entry
	{
		...
		var load = function(request)
		{
			switch(Number($system.dom.status(request.xml)))
			{
				...
				case 2 :
					var message = 'user/add/duplicate'
					break

				default :
					var message = 'user/add/fail'
					break
			}
		...
		return $system.network.send($self.info.root + 'server/php/run.php', {task: 'item.add'}, {
			name    : title,
	...
	this.get = function() //Get the list of todos
	{
		...
		var request = $system.network.send($self.info.root + 'server/php/run.php', {
			task  : 'item.get',
			filter: __display,
		}, null, null)
		...
		for(var i = 0; i < _todo.ordered.length; i++)
		{
			...
			for(var j = 0; j < header.length; j++)
			{
				...
				switch(header[j]) //Format the display according to column
				{
					...
					default :
						var display = $system.text.escape(value)
						break
				}
	...
	this.remove = function(id) //Remove an entry
	{
		...
		return $system.network.send($self.info.root + 'server/php/run.php', {task: 'item.remove'}, {id: id}, $system.app.method(update, [id]))
	}
	...
	this.set = function(form) //Set an item's information
	{
		...
		return $system.network.send($self.info.root + 'server/php/run.php', {task: 'item.set'}, param, $system.app.method(update, [form.id.value]))
	}
	...
	this.show = function(id) //Open the information window
	{
		...
		var replace = function(phrase, match)
		{
			...
			switch(match)
			{
				...
				case 'year' :
					return $system.date.select(Number(registered.year), Number(registered.year) + 10, false, value)
					break

				case 'month' :
					return $system.date.select(1, 12, false, value)
					break

				case 'day' :
					return $system.date.select(1, 31, false, value)
					break

				case 'hour' :
					return $system.date.select(0, 23, false, value)
					break

				case 'minute' :
					return $system.date.select(0, 59, false, value, true)
					break
