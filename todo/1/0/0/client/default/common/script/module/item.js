	...
	$self.item = new function()
	{
		...
		this.add = function(form) //Add a new entry
		{
			...
			return $system.network.send($self.info.root + 'server/php/run.php', {task : 'item.add'}, {name : title, category : form.category.value}, load);
		}
		...
		this.get = function() //Get the list of todos
		{
			...
			var request = $system.network.send($self.info.root + 'server/php/run.php', {task : 'item.get', filter : __display}, null, null);
