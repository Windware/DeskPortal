...
class Bookmark_1_0_0_Item
{
	protected static function _count($user, $id) #Update the address reference state from the user
	{
		...
		if($referred) #If reference exists, keep the count
		{
			...
			$query->run([':referred' => $referred, ':user' => $user->id, ':address' => $id]);
		}
		else #Otherwise delete the record
		{
			...
			$query->run([':user' => $user->id, ':address' => $id]);
		}
	...
	public static function add($address, System_1_0_0_User $user = null) #Add a new address
	{
		...
		if(!count($row) || (preg_match('|^text/html\b|i', $row['type']) || !$row['type']) && !strlen($row['title'])) #If not registered in the system address list or title was never entered
		{
			...
			$request = $system->network_http([['address' => $address, 'method' => 'HEAD', 'max' => Bookmark_1_0_0_Cache::$_max * 1000]]);

			if(preg_match('|^text/html\b|i', $request[0]['header']['content-type'])) #If HTML, get the title of the page
			{
				$request = $system->network_http([['address' => $address, 'max' => Bookmark_1_0_0_Cache::$_max * 1000]]);
			...
			if(!count($row)) #If the address does not exist, insert a new record
			{
				...
				$query->run([':address' => $address, ':title' => $title, ':type' => $request[0]['header']['content-type']]); #Add it
			...
			else #If the record exists with empty title, update the title
			{
				...
				$query->run([':title' => $title, ':type' => $request[0]['header']['content-type'], ':id' => $row['id']]);

				$id = $row['id'];
			}

			if(!$query->success) return false;
			$row = ['id' => $id, 'title' => $title]; #Get the inserted ID for the address field
		}
		...
		$query = $database->prepare("SELECT COUNT(id) FROM {$database->prefix}bookmark WHERE user = :user AND id = :id");
		$query->run([':user' => $user->id, ':id' => $row['id']]);
		...
		if(!$query->column()) #If not registered in the user's bookmark list, add it
		{
			...
			$query->run([':user' => $user->id, ':id' => $row['id'], ':added' => $system->date_datetime(), ':name' => $row['title']]);
		}
	...
	public static function get($cat = [], System_1_0_0_User $user = null) #List bookmarks
	{
		$system = new System_1_0_0(__FILE__);
		...
		$values = [':user' => $user->id]; #Query values
		...
		foreach($results as $row)
		{
			...
			$relation->run([':user' => $user->id, ':bookmark' => $row['id']]);
			...
			$cache->run([':user' => $user->id, ':bookmark' => $row['id'], ':file' => $info['address']]);
			if(!$cache->success) return false;

			$info['cache'] = $cache->column(); #Add the time the cache was acquired

			foreach(['added', 'name', 'viewed'] as $section) $info[$section] = $row[$section];
	...
	public static function import($file) #Import bookmark files
	{
		...
		$query = ['select' => $database['user']->prepare("SELECT id FROM {$database['user']->prefix}category WHERE user = :user AND name = :name")];
		...
		foreach($parser->list as $category => $link) #Check for category existence
		{
			if(strlen($category))
			{
				$query['select']->run([':user' => $user->id, ':name' => $category]);
				...
				if(!$id)
				{
					$query['insert']->run([':user' => $user->id, ':name' => $category]); #Save the new category
		...
		$query = ['add' => $database['system']->prepare("INSERT INTO {$database['system']->prefix}address (address) VALUES (:address)")];
		...
		for($i = 0; $i < count($all); $i += $database['system']->limit)
		{
			...
			foreach(array_diff($list, $exist) as $id) #Insert bookmark not in the user database
			{
				...
				$query['insert']->run([':user' => $user->id, ':id' => $id, ':added' => $time, ':name' => $name[$info[$id]['address']]]);
				if(!$query['insert']->success) return false;

				foreach(array_unique($relation[$id]) as $category)
				{
					$query['category']->run([':user' => $user->id, ':bookmark' => $id, ':category' => $category]); #Link bookmark into categories
		...
		{
			$query['add']->run([':address' => $address]); #Insert into system database
			if(!$query['add']->success) return false;

			$id = $database['system']->id(); #Get the ID of the bookmark
			if(!$id || !$database['user']->begin()) return false;

			$query['insert']->run([':user' => $user->id, ':id' => $id, ':added' => $time, ':name' => $name]); #Insert into user database
			if(!$query['insert']->success) return false;
	...
	public static function remove($id, System_1_0_0_User $user = null) #Remove a bookmark
	{
		...
		$query->run([':user' => $user->id, ':bookmark' => $id]); #Remove the category relation

		if(!$query->success) return false;

		$query = $database->prepare("DELETE FROM {$database->prefix}bookmark WHERE user = :user AND id = :id");
		$query->run([':user' => $user->id, ':id' => $id]); #Remove the bookmark entry
	...
	public static function set($address, $name, $categories, $id, System_1_0_0_User $user = null) #Sets a bookmark's data
	{
		...
		$query = $database->prepare("DELETE FROM {$database->prefix}relation WHERE user = :user AND bookmark = :bookmark");
		$query->run([':user' => $user->id, ':bookmark' => $id]); #Delete the old category and bookmark relation
		...
		{
			$query = $database->prepare("DELETE FROM {$database->prefix}bookmark WHERE user = :user AND id = :id");
			$query->run([':user' => $user->id, ':id' => $id]); #Delete the old bookmark
			...
			$query->run([':name' => $name, ':added' => $result['added'], ':viewed' => $result['viewed'], ':user' => $user->id, ':id' => $id]);
		...
		elseif($name != $result['name']) #When address is kept intact and the name has changed
		{
			...
			$query->run([':name' => $name, ':user' => $user->id, ':id' => $id]); #Update its name
		...
		if(count($categories))
		{
			...
			foreach($categories as $cat) #For all of the provided categories
			{
				...
				$query->run([':user' => $user->id, ':bookmark' => $id, ':category' => $cat]); #Relate it to the bookmark
	...
	public static function viewed($id, System_1_0_0_User $user = null) #Increase the view count of a page
	{
		...
		$query->run([':viewed' => $viewed, ':user' => $user->id, ':id' => $id]);
