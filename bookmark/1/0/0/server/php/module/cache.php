<?php
	class Bookmark_1_0_0_Cache
	{
		...
		public static function add($id, System_1_0_0_User $user = null) #Create a cache of a bookmark
		{
			...
			$query->run(array(':id' => $id)); #Get the address of the bookmark
			...
			$query->run(array(':user' => $user->id, ':bookmark' => $id, ':file' => $address, ':time' => $system->date_datetime(), ':type' => $request[0]['header']['content-type'], ':content' => $system->compress_build($page)));
