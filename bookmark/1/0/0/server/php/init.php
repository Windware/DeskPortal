<?php
	class Bookmark_1_0_0
	{
		...
		{ #TODO - Not used in this version
			...
			$query->run(array(':date' => $system->date_datetime(time() - 24 * 3600))); #Pick entries not checked in the last 24 hours

			if(!$query->success) return false;
			#TODO - Check the number of reference to the bookmark and if no one is using it, delete it or skip it
			$pool = $target = $id = array(); #List of connections, addresses and address ID

			foreach($query->all() as $row)
			{
				$target[] = array('address' => $row['address'], 'method' => 'HEAD');
