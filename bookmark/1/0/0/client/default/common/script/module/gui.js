$self.gui = new function()
{
	...
	var _list = function(id) //Construct category list HTML
	{
		...
		for(var i = 0; i < __group.ordered.length; i++) //FIXME - Get the HTML as a template
		{
			...
			category += $system.text.format(text, {
				bookmark: id,
				cat     : cat.id,
				name    : $system.text.escape(cat.name),
				id      : $id,
				check   : checked,
			})
		}
	...
	this.cache = function(id) //Create the cache content window of a bookmark
	{
		...
		var values = {
			id   : id,
			title: __bookmarks[id].name,
			cache: $system.network.form($self.info.root + 'server/php/run.php?task=gui.cache&id=' + id),
		}
