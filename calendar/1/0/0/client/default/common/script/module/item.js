	...
	$self.item = new function()
	{
		...
		this.remove = function(year, month, day) //Removes the schedule off a day
		{
			...
			return $system.network.send($self.info.root + 'server/php/run.php', {task : 'item.remove'}, {year : year, month : month, day : day}, update);
		}
		...
		this.set = function(year, month, day, discard, initial) //Apply the edited information for the day
		{
			...
			return $system.network.send($self.info.root + 'server/php/run.php', {task : 'item.set'}, post, $system.app.method(notify, [cell, date]));
		}
