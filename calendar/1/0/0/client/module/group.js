	...
	$self.group = new function()
	{
		...
		this.remove = function() //Removes the chosen group
		{
			...
			return $system.network.send($self.info.root + 'server/php/front.php', {task : 'group.remove'}, {category : category}, removal);
		}
		...
		this.set = function(form) //Submit the category edit
		{
			...
			$system.network.send($self.info.root + 'server/php/front.php', {task : 'group.set'}, values, $system.app.method(refresh, [form]));
