	...
	$self.login = new function()
	{
		...
		this.request = function(identity, pass, keep, callback) //Make a login request remotely
		{
			...
			$system.network.send($self.info.root + 'server/php/run.php', {task : 'login.process', identity : identity, pass : pass, keep : keep ? 1 : 0}, null, $system.app.method(process, [callback, keep]));
		}
	}
