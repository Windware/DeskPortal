<?php
	class Login_1_0_0
	{
		...
		public static function attempt($name, $pass, $keep) #Return codes are (0 : sucess, 1 : fail, 2 : error)
		{
			...
			#Issue the cookies if session could be recorded on the server side
			#NOTE : Passing both the ticket and the user name instead of just the ticket to make it difficult to brute force attack on ticket value
		...
		#Check if the login succeeds or not (And create the user if necessary) and returns the user ID
		...
		{ #TODO - limit amount of login attempt in a certain time frame (ex : 5 times within 3 min)
			...
			if(!System_Static_Auth::verify($name, $pass)) #Look up for authorization
			...
			return $conf['user_create'] ? $system->user_create($name, $pass) : $system->user_find($name);
		}
