	...
	$self.gui = new function()
	{
		...
		this.page = function(search, name, page) //Change the result page of a section
		{
			...
			$system.network.send($self.info.root + 'server/php/run.php', {task : 'gui.page', search : search, area : [name], page : page}, null, $system.app.method(_display, [zone, true, log]));
		}
		...
		{ //TODO - Calculate the percentage of probability sorted (hit / most hit %) and have option only to show higher probability hits
			...
			$system.network.send($self.info.root + 'server/php/run.php', {task : 'gui.search', area : area, search : _search}, null, $system.app.method(_display, [zone, false, log]));
