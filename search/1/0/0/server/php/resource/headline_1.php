...
class Search_1_0_0_Resource_Headline_1
{
	...
	public function __construct($phrase, $limit, $page, System_1_0_0_User $user = null)
	{
		...
		$query->run([':user' => $user->id]); #Search for all subscribed feeds

		if(!$query->success) return false;

		$sub = []; #List of subscription
		...
		foreach(array_slice($all, ($page - 1) * $limit, $limit) as $row) $this->result['item'][] = ['id' => $row['id'], 'date' => preg_replace('/ .+/', '', $row['date']), 'text' => $row['subject']];
	}
}
