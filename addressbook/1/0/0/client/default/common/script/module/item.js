	...
	$self.item = new function()
	{
		...
		this.edit = function(id) //Pop a window to edit an entry
		{
			...
			var list = function(id) //Do post load operation of the edit window
			{
				...
				else
				{
					...
					{
						case '0' : $system.node.id($id + '_gender_female_' + id).checked = true; break;

						case '1' : $system.node.id($id + '_gender_male_' + id).checked = true; break;
					}
				}
		...
		this.get = function(group, refresh, callback) //Show the items
		{
			...
			return $system.network.send($self.info.root + 'server/php/run.php', {task : 'item.get', group : group}, null, $system.app.method(display, [callback]));
		}
