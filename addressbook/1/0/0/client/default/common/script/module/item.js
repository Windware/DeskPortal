$self.item = new function()
{
	...
	this.edit = function(id) //Pop a window to edit an entry
	{
		...
		var list = function(id) //Do post load operation of the edit window
		{
			...
			else
			{
				...
				switch(_address[id].gender)
				{
					case '0' :
						$system.node.id($id + '_gender_female_' + id).checked = true
						break

					case '1' :
						$system.node.id($id + '_gender_male_' + id).checked = true
						break
				}
			}
	...
	this.get = function(group, refresh, callback) //Show the items
	{
		...
		var display = function(callback, request)
		{
			...
			if(request === undefined)
			{
				...
				if($system.browser.name == 'Safari' && $system.browser.version >= 4)
				{ if(typeof _selection.request != 'function') wrong = true }
			...
			for(var i = 0; i < list.length; i++)
			{
				...
				for(var j = 0; j < nodes.length; j++) //Load up the attributes
				{
					...
					{
						var birth = {
							month: $system.dom.attribute(list[i], 'birth_month') || '?',
							day  : $system.dom.attribute(list[i], 'birth_day') || '?',
						}
				...
				for(var j = 0; j < __all.length; j++) //Sustain the order by looking at the master list
				{
					...
					switch(__all[j])
					{
						...
						case 'gender' : //Show textual description
	...
	this.set = function(id, form) //Sets an entry information
	{
		...
		$system.network.send($self.info.root + 'server/php/run.php', {task: 'item.set'}, params, callback)
